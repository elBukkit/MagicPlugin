...
public abstract class EffectPlayer implements com.elmakers.mine.bukkit.api.effect.EffectPlayer {
    ...
    public void load(Plugin plugin, ConfigurationSection configuration) {
        ...
        if (configuration.contains("particle")) {
            ...
            particleType = CompatibilityLib.getCompatibilityUtils().getParticle(typeName);
            if (particleType == null) {
                ...
                if (particleOffsetVector != null) {
                    particleXOffset = (float)particleOffsetVector.getX();
                    particleYOffset = (float)particleOffsetVector.getY();
                    particleZOffset = (float)particleOffsetVector.getZ();
                } else {
    ...
    protected MaterialAndData getWorkingMaterial() {
        ...
        if (sampleTarget && target != null) {
            return new MaterialAndData(target.getBlock().getType());
        }
        if (material1 != null) return material1;
        MaterialAndData result = material;
        Location origin = getOrigin();
        if (result == null && target != null) {
            result = new MaterialAndData(target.getBlock().getType());
        } else if (result == null && origin != null) {
            result = new MaterialAndData(origin.getBlock().getType());
        } else if (result == null) {
