...
public class Magic
{
    ...
    public void addToUndoQueue(Player player, BlockList blocks)
    {
        ...
        /*
         * TODO: Get this working again! if (autoExpandUndo) { BlockList
         * expandedBlocks = new BlockList(blocks); for (UndoableBlock undoBlock
         * : blocks.getBlocks()) { Block block = undoBlock.getBlock(); Material
         * newType = block.getType(); if (newType ==
         * undoBlock.getOriginalMaterial() || isSolid(newType)) { continue; }
         * 
         * for (int side = 0; side < 4; side++) { BlockFace sideFace =
         * UndoableBlock.SIDES[side]; Block sideBlock = block.getFace(sideFace);
         ...
         * 
         * Material topMaterial = undoBlock.getOriginalTopMaterial(); Block
         * topBlock = block.getFace(BlockFace.UP); if
         * (!blocks.contains(topBlock)) { if (isAffectedByGravity(topMaterial))
         * { expandedBlocks.addBlock(topBlock); if (autoPreventCaveIn) {
         * topBlock.setType(gravityFillMaterial); } else { for (int dy = 0; dy <
         * undoCaveInHeight; dy++) { topBlock = topBlock.getFace(BlockFace.UP);
         * if (isAffectedByGravity(topBlock.getType())) {
         * expandedBlocks.addBlock(topBlock); } else { break; } } } } else if
         * (isStickyAndTall(topMaterial)) { UndoableBlock stickyBlock =
         * expandedBlocks.addBlock(topBlock);
         * stickyBlock.setFromBottom(undoBlock); stickyBlock =
         * expandedBlocks.addBlock(topBlock.getFace(BlockFace.UP));
         * stickyBlock.setFromBottom(undoBlock); } else if
         * (isSticky(topMaterial)) { UndoableBlock stickyBlock =
         * expandedBlocks.addBlock(topBlock);
         * stickyBlock.setFromBottom(undoBlock); } } } blocks = expandedBlocks;
         * }
         */
        queue.add(blocks);
    }
    ...
    {
        /*
         * addSpell(new HealSpell()); addSpell(new BlinkSpell()); addSpell(new
         * TorchSpell()); addSpell(new FireballSpell()); addSpell(new
         * PillarSpell()); addSpell(new BridgeSpell()); addSpell(new
         * AbsorbSpell()); addSpell(new FillSpell()); addSpell(new
         * CushionSpell()); addSpell(new UndoSpell()); addSpell(new
         * AlterSpell()); addSpell(new BlastSpell()); addSpell(new MineSpell());
         * addSpell(new TreeSpell()); addSpell(new ArrowSpell()); addSpell(new
         * FrostSpell()); addSpell(new GillsSpell()); addSpell(new
         * FamiliarSpell()); addSpell(new ConstructSpell()); addSpell(new
         * TransmuteSpell()); addSpell(new RecallSpell()); addSpell(new
         * DisintegrateSpell()); addSpell(new ManifestSpell()); addSpell(new
         * PeekSpell()); addSpell(new FireSpell()); addSpell(new LavaSpell());
         * addSpell(new InvincibleSpell()); addSpell(new TunnelSpell());
         * 
         * // wip // addSpell(new TowerSpell()); // addSpell(new ExtendSpell());
         * // addSpell(new StairsSpell());
         * 
         * // NetherGate spells if (nether != null) { addSpell(new
