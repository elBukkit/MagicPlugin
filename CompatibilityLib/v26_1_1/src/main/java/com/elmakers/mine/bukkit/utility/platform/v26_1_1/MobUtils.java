package com.elmakers.mine.bukkit.utility.platform.v26_1_1;
...
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.IdleGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.MagicCheckOwnerGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.MagicFindOwnerGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.MagicFollowMobGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.MagicFollowOwnerGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.MagicGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.MagicOwnerHurtByTargetGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.MagicOwnerHurtTargetGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.MagicPanicGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.MagicTemptGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.RequirementsGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.SpinGoal;
import com.elmakers.mine.bukkit.utility.platform.v26_1_1.goal.TriggerGoal;
...
public class MobUtils extends MobUtilsBase {
    ...
    protected boolean removeGoal(GoalSelector selector, Mob mob, Entity entity, GoalType goalType) {
        ...
        try {
            ...
            platform.getLogger().log(Level.WARNING, "Error removing goal: " + goalType + " from " + entity.getType(),
                    ex);
            return false;
        }
        return true;
    ...
    public String getGoalParentDescriptions(Goal goal) {
        ...
        return ChatColor.DARK_GRAY + " -> " + ChatColor.GRAY
                + StringUtils.join(parentClasses, ChatColor.DARK_GRAY + " -> " + ChatColor.GRAY);
    }
    ...
    protected Collection<String> getGoalDescriptions(GoalSelector selector) {
        ...
        for (WrappedGoal wrappedGoal : available) {
            ...
            descriptions.add(ChatColor.BLUE + Integer.toString(wrappedGoal.getPriority()) + ChatColor.DARK_GRAY + ": "
                    + description);
        }
        return descriptions;
    }
    ...
    protected boolean addGoal(GoalSelector selector, Mob mob, Entity entity, GoalType goalType, int priority,
    ...
    private Goal getGoal(GoalType goalType, Entity entity, Mob mob, ConfigurationSection config) {
        ...
        switch (goalType) {
            ...
            case FOLLOW_BOAT:
                if (pathfinder == null)
                    return null;
                return new FollowPlayerRiddenEntityGoal(pathfinder, Boat.class);
            ...
            case FOLLOW_OWNER:
                if (mob instanceof TamableAnimal) {
                    return new FollowOwnerGoal((TamableAnimal) mob, speed, startDistance, stopDistance);
                }
            ...
            case INTERACT:
                return new InteractGoal(mob, getMobClass(classType), distance,
                        (float) config.getDouble("probability", 1));
            ...
            case LOOK_AT_PLAYER:
                return new LookAtPlayerGoal(mob, getMobClass(classType), distance,
                        (float) config.getDouble("probability", 1), config.getBoolean("horizontal"));
            case MELEE_ATTACK:
                if (pathfinder == null)
                    return null;
            ...
            case MOVE_THROUGH_VILLAGE:
                if (pathfinder == null)
                    return null;
                return new MoveThroughVillageGoal(pathfinder, speed, config.getBoolean("night", true), (int) distance,
                        (BooleanSupplier) () -> doors);
            ...
            case MAGIC_PANIC:
                if (pathfinder == null)
                    return null;
                return new MagicPanicGoal(pathfinder, speed, config.getInt("panic", 3000), config.getInt("calm", 5000),
                        interruptable);
    ...
    private List<Goal> getGoals(Entity entity, Mob mob, ConfigurationSection config, String logContext) {
        ...
        List<GoalConfiguration> goalConfigurations = GoalConfiguration.fromList(config, "goals", platform.getLogger(),
                logContext);
            ...
            for (GoalConfiguration goalConfig : goalConfigurations) {
                try {
                    ...
                    platform.getLogger().log(Level.WARNING,
                            "Error creating goal: " + goalConfig.getGoalType() + " on mob " + entity.getType(), ex);
                }
            }
        }
        if (goals.isEmpty()) {
