...
{    
    ...
    public void onPlayerAnimation(PlayerAnimationEvent event) 
	{
		...
		{
			if (event.getPlayer().getInventory().getItemInHand().getType() == getWandType())
			{
				...
				for (int i = 0; i < 9; i++)
				{
					if (contents[i].getType() == Material.AIR || contents[i].getType() == getWandType())
					{
						continue;
					}
					
					spell = magic.getSpell(contents[i].getType(), player);
					if (spell != null)
					{
						break;
					}
	...
	public void cycleSpells(Player player)
	{
		...
		for (int i = 0; i < 9; i++)
		{
			...
			{
				SpellVariant spell = null;//magic.getSpell(active[i].getType(), player);
				isSpell = spell != null;
			}
	...
	public void spellHelp(Player player)
	{
		...
		for (int i = 0; i < 9; i++)
		{
			...
			{
				SpellVariant ispell = magic.getSpell(contents[i].getType(), player);
